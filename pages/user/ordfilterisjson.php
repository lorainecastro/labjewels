    ...
    foreach ($xml->order as $order) {
        if ((int)$order->user_id == $currentUser['user_id']) {
            ...
            $orderData = [
                ...
                'status' => isset($order->status) ? trim(strtolower((string)$order->status)) : 'pending', // Normalize status
...
$ordersPerPage = 2;
$currentFilter = isset($_GET['status']) ? trim(strtolower(htmlspecialchars($_GET['status']))) : 'all';
$currentPage = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1;

// Validate filter
$validFilters = ['all', 'pending', 'shipped', 'delivered', 'cancelled'];
...
$totalPages = ceil($totalFilteredOrders / $ordersPerPage) ?: 1;
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .filter-btn.active + .notification-badge {
            background-color: var(--primary-color);
            color: var(--whitefont-color);
        }
...
<body>
    <div class="orders-section constraints">
        ...
        <?php else: ?>
            <div id="orderCards">
                <?php if (empty($paginatedOrders)): ?>
                    <div class="empty-state">
                        ...
                        <?php
                        $statusMessages = [
                            'all' => ['title' => 'No Orders Yet', 'message' => 'You haven\'t placed any orders yet. Start shopping to see your orders here!'],
                            'pending' => ['title' => 'No Pending Orders', 'message' => 'You have no pending orders at the moment. Check back later or start shopping!'],
                            'shipped' => ['title' => 'No Shipped Orders', 'message' => 'You have no shipped orders currently. Your orders will appear here once they are shipped.'],
                            'delivered' => ['title' => 'No Delivered Orders', 'message' => 'You have no delivered orders yet. Your orders will appear here once they are delivered.'],
                            'cancelled' => ['title' => 'No Cancelled Orders', 'message' => 'You have no cancelled orders. Your orders will appear here if any are cancelled.']
                        ];
                        $currentStatus = $statusMessages[$currentFilter] ?? $statusMessages['all'];
                        ?>
                        <h2><?php echo htmlspecialchars($currentStatus['title']); ?></h2>
            ...
            <?php if ($totalPages > 1 && !empty($paginatedOrders)): ?>
    ...
    <script>
        ...
        // Debug: Log orders to verify data
        ...
        function openOrderDetailsModal(button) {
            try {
                ...
                document.getElementById('timestampDisplay').textContent = new Date(order.timestamp).toLocaleString('en-US', { month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function updatePagination(filteredOrders, page) {
                ...
                paginationContainer.style.display = filteredOrders.length === 0 ? 'none' : 'flex';

                // Update URL without reloading
                const newUrl = `?status=${encodeURIComponent(currentFilter)}&page=${currentPage}`;
                history.pushState({ filter: currentFilter, page: currentPage }, '', newUrl);
            }

            function applyFilterAndPagination(filter, page) {
                currentFilter = filter.toLowerCase().trim(); // Normalize filter
                currentPage = page;

                // Debug: Log filter and page
                ...
                filterSection.innerHTML = `
                    <div class="filter-btn-wrapper">
                        <button class="filter-btn ${currentFilter === 'all' ? 'active' : ''}" data-status="all">
                            All Orders
                        </button>
                        <span class="notification-badge">${totalOrders}</span>
                    </div>
                    <div class="filter-btn-wrapper">
                        <button class="filter-btn ${currentFilter === 'pending' ? 'active' : ''}" data-status="pending">
                            Pending
                        </button>
                        <span class="notification-badge">${statusCounts.pending}</span>
                    </div>
                    <div class="filter-btn-wrapper">
                        <button class="filter-btn ${currentFilter === 'shipped' ? 'active' : ''}" data-status="shipped">
                            Shipped
                        </button>
                        <span class="notification-badge">${statusCounts.shipped}</span>
                    </div>
                    <div class="filter-btn-wrapper">
                        <button class="filter-btn ${currentFilter === 'delivered' ? 'active' : ''}" data-status="delivered">
                            Delivered
                        </button>
                        <span class="notification-badge">${statusCounts.delivered}</span>
                    </div>
                    <div class="filter-btn-wrapper">
                        <button class="filter-btn ${currentFilter === 'cancelled' ? 'active' : ''}" data-status="cancelled">
                            Cancelled
                        </button>
                        <span class="notification-badge">${statusCounts.cancelled}</span>
                    </div>
                `;
                ...
                console.log('Paginated Orders:', paginatedOrders); // Debug: Log paginated orders

                // Generate order cards HTML or empty state
                let cardsHTML = '';
                if (paginatedOrders.length === 0) {
                    ...
                    const currentStatus = statusMessages[currentFilter] || statusMessages['all'];
                    cardsHTML = `
                        <div class="empty-state">
                            <i class="fas fa-clipboard-list"></i>
            ...
            window.addEventListener('popstate', (e) => {
                const state = e.state || { filter: 'all', page: 1 };
