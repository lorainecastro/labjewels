...
if ($xml && isset($xml->order)) {
    foreach ($xml->order as $order) {
        ...
        if (!$total_amount && isset($order->items->item)) {
            // Convert SimpleXMLElement to array to handle single or multiple items
                ...
                <tbody id="ordersTableBody">
                    ...
                    if (!empty($orders)) {
                        foreach ($orders as $index => $order) {
                            ...
                            echo "<td>PHP " . number_format($order['total_amount'], 2) . "</td>";
                            echo "<td class='status " . htmlspecialchars($order['status']) . "'>" . ucfirst(htmlspecialchars($order['status'])) . "</td>";
