...
// Get categories
$categories = [];
if (isset($productsXml->categories->category)) {
    foreach ($productsXml->categories->category as $category) {
...
foreach ($ordersXml->order as $order) {
    ...
    foreach ($order->items->item as $item) {
        ...
        $orderSubtotal += $itemData['price'] * $itemData['quantity'];
    ...
    $orderTotal = $orderSubtotal + $orderData['shipping_fee'];
    $totalShippingFees += $orderData['shipping_fee']; // Sum shipping fees for all orders
    if (in_array(strtolower($orderData['status']), ['shipped', 'delivered'])) {
        $totalSales += $orderTotal; // Include total (subtotal + shipping) for shipped/delivered
        $totalSalesExcludingShipping += $orderSubtotal; // Include only subtotal for shipped/delivered
    }
    $orders[] = $orderData;
...
if ($categoryFilter) {
    $filteredOrders = array_filter($filteredOrders, function ($order) use ($categoryFilter, $productsXml) {
        foreach ($order['items'] as $item) {
            foreach ($productsXml->products->product as $product) {
                if ((int)$product->id === $item['product_id'] && (string)$product->category === $categoryFilter) {
                    return true;
                }
            }
        }
...
if ($productFilter) {
    $filteredOrders = array_filter($filteredOrders, function ($order) use ($productFilter) {
        foreach ($order['items'] as $item) {
            if ((int)$item['product_id'] === (int)$productFilter) {
                return true;
            }
        }
        return false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'export_pdf') {
    ...
    $html .= '<p><strong>Net Sales:</strong> PHP ' . number_format($totalSalesExcludingShipping, 2) . '</p>';
    $html .= '<p><strong>Shipping Fees:</strong> PHP ' . number_format($totalShippingFees, 2) . '</p>';
    ...
    $html .= '<h2>Top Products</h2><table border="1" cellpadding="5"><thead><tr style="background-color: #f0f0f0;"><th>Product ID</th><th>Name</th><th>Units Sold</th></tr></thead><tbody>';
    foreach ($topProducts as $productId => $quantity) {
        foreach ($productsXml->products->product as $product) {
            if ((int)$product->id === $productId) {
                $html .= "<tr><td>$productId</td><td>" . htmlspecialchars($product->name) . "</td><td>$quantity</td></tr>";
                break;
            }
        }
    }
    ...
    $html .= '<h2>Filtered Orders</h2><table border="1" cellpadding="5"><thead><tr style="background-color: #f0f0f0;"><th>Order ID</th><th>User ID</th><th>Date</th><th>Payment Method</th><th>Status</th><th>Total</th></tr></thead><tbody>';
    foreach ($filteredOrders as $order) {
        ...
        $html .= "<tr><td>" . htmlspecialchars($order['order_id']) . "</td><td>" . $order['user_id'] . "</td><td>" . $order['timestamp'] . "</td><td>" . htmlspecialchars($order['payment_method']) . "</td><td>" . $order['status'] . "</td><td>PHP " . number_format($orderTotal, 2) . "</td></tr>";
    }
    $html .= '</tbody></table>';
...
<head>
    ...
    <title>LAB Jewels - Reports</title>
...
<body>
    <div class="container">
        ...
        <div class="dashboard-grid">
            ...
            <div class="card">
                <div class="card-header">
                    <div>
                        ...
                        <div class="card-value">₱<?php echo number_format($totalSalesExcludingShipping, 2); ?></div>
                    </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div>
                        <div class="card-title">Shipping Fees</div>
                        <div class="card-value">₱<?php echo number_format($totalShippingFees, 2); ?></div>
                    </div>
            ...
            <div class="filter-grid">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="statusFilter">
                        ...
                        <option value="delivered" <?php echo $dateRangeFilter === 'delivered' ? 'selected' : ''; ?>>Delivered</option>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <tbody id="orderTableBody">
                        ...
                        if (!empty($filteredOrders)) {
                            foreach ($filteredOrders as $order) {
                                ...
                                $itemsList = '';
                                foreach ($order['items'] as $item) {
                                    $itemsList .= htmlspecialchars($item['product_name']) . ' (' . $item['quantity'] . '), ';
                                    $orderTotal += $item['price'] * $item['quantity'];
                                }
                                ...
                                echo "<td>" . date('Y-m-d', strtotime($order['timestamp'])) . "</td>";
                                echo "<td>" . $itemsList . "</td>";
                                echo "<td>" . htmlspecialchars($order['payment_method']) . "</td>";
                                echo "<td><span class='status " . strtolower($order['status']) . "'>" . ucfirst($order['status']) . "</span></td>";
    ...
    <script>
        const orders = <?php echo json_encode($filteredOrders); ?>;
        ...
        const statusData = {
            day: {
                ...
                datasets: [{
                        label: 'Pending',
                        data: [<?php echo $statusCounts['pending']; ?>],
                    ...
                    {
                        label: 'Shipped',
                        data: [<?php echo $statusCounts['shipped']; ?>],
                    ...
                    {
                        label: 'Delivered',
                        data: [<?php echo $statusCounts['delivered']; ?>],
                    ...
                    {
                        label: 'Cancelled',
                        data: [<?php echo $statusCounts['cancelled']; ?>],
